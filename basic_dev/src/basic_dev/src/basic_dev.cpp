...
{  
    ...
    nh->param("max_vy", max_vy_, 14.0);                      // 路径跟踪时机体系Y轴速度上限（m/s）
    nh->param("max_vz", max_vz_, 12.0);                 // 路径跟踪时机体系Z轴上升速度上限（m/s）
    nh->param("max_vz_start_zone", max_vz_start_zone_, 2.6); // 起点后前段Z速度上限（m/s）

    // CSV轨迹跟踪参数
    nh->param("csv_path", csv_path_, std::string("/home/peter/xwechat_files/wxid_0r5rz9iaczoa22_5e98/msg/file/2026-03/gnss_path.csv")); // 轨迹CSV文件路径
    nh->param("downsample_dist", downsample_dist_, 0.8);   // 轨迹点降采样间距（m）
    nh->param("lookahead_dist", lookahead_dist_, 6.0);    // 前视距离（m），越大越平滑，越小越贴线
    ...
    nh->param("invert_csv_y", invert_csv_y_, true);        // 是否对CSV的Y轴取反（用于坐标系左右镜像修正）
    nh->param("fp_kp_xy", fp_kp_xy_, 1.8);               // 兼容旧参数：平面P增益（默认同步为X轴）
    nh->param("fp_kp_x", fp_kp_x_, 1.8);                 // 机体系X轴位置误差增益
    nh->param("fp_kp_y", fp_kp_y_, 5.2);                 // 机体系Y轴位置误差增益
    nh->param("lateral_slow_err_thresh", lateral_slow_err_thresh_, 6.0); // 横向偏差超过阈值才降前向速度（m）
    ...
    nh->param("fp_kd_yaw_rate", fp_kd_yaw_rate_, 0.22);    // 航向D增益：抑制过冲（基于实测偏航角速度）
    nh->param("yaw_rate_lpf_alpha", yaw_rate_lpf_alpha_, 0.32); // 偏航角速度低通滤波系数(0~1)
    nh->param("yaw_soft_limit_deg", yaw_soft_limit_deg_, 120.0); // yawRate软限幅（deg/s，与yaw_tuner快档一致）
    ...
    nh->param("yaw_only_deg", yaw_only_deg_, 50.0);        // 启用门控时的阈值（deg）
...
void BasicDev::enter_phase(TestPhase p, const ros::Time& now)
{
    ...
    if (phase == TestPhase::FOLLOW_PATH) {
...
void BasicDev::path_follow_step(double& vx, double& vy, double& vz, double& yaw_deg, bool& finished)
{
    ...
    const size_t i_tar  = find_lookahead_index(i_near, lookahead_dist_);
    const Eigen::Vector3d p_tar = path_pts_[i_tar];
    ...
    vx = clampd(fp_kp_x_ * e_b.x(), -max_vx_, max_vx_);
    vy = clampd(fp_kp_y_ * e_b.y(), -max_vy_, max_vy_);
    // 小改动：只在横向偏差超过阈值时才降低前向推进，避免全程被拖慢
    double vx_lateral_scale = 1.0;
    const double ey_abs = std::fabs(e_b.y());
    if (ey_abs > lateral_slow_err_thresh_) {
        const double ey_excess = ey_abs - lateral_slow_err_thresh_;
    ...
    if (dist_end < reach_end_dist_) finished = true;

    ROS_INFO_THROTTLE(0.5, "[PATH] near=%zu tar=%zu tar_z=%zu yaw_err=%.1f wccw=%.1f dz=%.2f vref=%.2f vz=%.2f zi=%.2f vd=%.2f dist_end=%.2f e_b=(%.2f %.2f %.2f) cmd=(%.2f %.2f %.2f %.1f)",
                      i_near, i_tar, i_tar_z, yaw_err_deg, yaw_rate_meas_ccw, dz_w, v_ref, z_vel_meas_filt_, z_pid_int_, z_pid_d_filt_,
                      dist_end, e_b.x(), e_b.y(), e_b.z(), vx, vy, vz, yaw_deg);
}
...
{
    // GPS 仅保留观测日志，不再用于控制状态更新（控制改用 pose_gt）
...
void BasicDev::lidar_cb(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
    ...
    if (!corridor_enable_ || !has_gps || !corridor_cloud_ || corridor_voxel_leaf_ <= 1e-6) {
        return;
    }

    const Eigen::Quaterniond q_wb = current_quat.normalized();
    const Eigen::Vector3d t_w = current_pos;
    const double leaf = corridor_voxel_leaf_;
    ...
    for (const auto& p : pts->points) {
        ...
        const int ix = static_cast<int>(std::floor(p_w.x() / leaf));
        const int iy = static_cast<int>(std::floor(p_w.y() / leaf));
        const int iz = static_cast<int>(std::floor(p_w.z() / leaf));
        const long long key = pack_voxel_key(ix, iy, iz);

        if (corridor_voxel_keys_.find(key) != corridor_voxel_keys_.end()) continue;
        if (corridor_cloud_->points.size() >= corridor_max_points_) continue;

        corridor_voxel_keys_.insert(key);
        pcl::PointXYZ pw;
        pw.x = static_cast<float>(p_w.x());
        pw.y = static_cast<float>(p_w.y());
        pw.z = static_cast<float>(p_w.z());
        corridor_cloud_->points.push_back(pw);
    }
    ...
    if (last_corridor_pub_time_.isZero() || (now - last_corridor_pub_time_).toSec() >= min_pub_period) {
        ...
        pcl::toROSMsg(*corridor_cloud_, map_msg);
        map_msg.header.stamp = now;
        map_msg.header.frame_id = viz_frame_id_;
        corridor_map_pub_.publish(map_msg);
        last_corridor_pub_time_ = now;
        ROS_INFO_THROTTLE(1.0, "Corridor map points: %zu", corridor_cloud_->points.size());
    }
}
